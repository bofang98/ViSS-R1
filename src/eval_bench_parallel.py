...
def evaluate_dataset(output_path_root, dataset_name, llm, sampling_params, processor, file_name, BSZ=64):
    ...
    # OUTPUT_PATH = f"./src/r1-v/eval_results_mask/eval_{dataset_name}_{file_name}_{MASK_RATIO}.json"
    # OUTPUT_PATH = f"/ssd2/vis/wuwenhao/fangbo/zzzzz/eval_{dataset_name}_{file_name}.json"

    if not os.path.exists(os.path.join(output_path_root, "eval_results")):
        os.makedirs(os.path.join(output_path_root, "eval_results"), exist_ok=True)

    OUTPUT_PATH = os.path.join(output_path_root, f"eval_results/{dataset_name}_{file_name}.json")
    ...
    #     "You will be given a video along with an user question. On the basis of the video content, answer the user question. "
    #     "Please think through the question step by step, as a human would. Carefully analyze the visual content and reason in detail. "
    ...
    #     "Put your detailed thought process inside <think>...</think> tags. Place your final answer to the user question inside <answer>...</answer> tags. For example: <think> ... </think><answer> ... </answer>.\n"
    #     "Important:\n"
    ...
    for x in data:
        ...
        msg = [{
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": QUESTION_TEMPLATE.format(Question=question) + TYPE_TEMPLATE[x['problem_type']]        # video-r1 style prompt
                    # "text": QUESTION_TEMPLATE.format(Question=question, Type_template=TYPE_TEMPLATE[x['problem_type']])     # my-ssl prompt
                }
            ]
        }]
        messages.append(msg)
    ...
    if os.path.exists(OUTPUT_PATH):
        # print(f"[{dataset_name}] Output file already exists at {OUTPUT_PATH}, break down")
        try:
    ...
    for i in tqdm(range(start_idx, len(messages), BSZ), desc=f"[{dataset_name}] Processing"):
        ...
        try:
            image_inputs, video_inputs, video_kwargs = process_vision_info(batch_messages, return_video_kwargs=True)        # [BSZ x [32, 3, H, W]]

            # apply random transformation for videos 
            # new_video_inputs = []
            # for i in range(BSZ):
            #     trans_video, trans_gt, trans_question = process_video(video_inputs[i])
            #     new_video_inputs.append(trans_video[0])
            # video_inputs = new_video_inputs

            # randomly mask frames and inference
            # new_video_inputs = []
            # for idx_bv in range(len(video_inputs)):
            #     mask_video = mask_video_frames(video_inputs[idx_bv], ratio=MASK_RATIO)      # mask frames
            #     new_video_inputs.append(mask_video)
            # video_inputs = new_video_inputs
...
def process_shuffle(video):
    ...
    """
    assert video.shape[0] == 32, "video lenght is not 32"
    groups = [
...
def mask_video_frames(video, ratio):
    num_frames = video.shape[0]
    num_mask = int(num_frames * ratio)

    mask_indices = np.random.choice(num_frames, num_mask, replace=False)
    keep_indices = np.array([i for i in range(num_frames) if i not in mask_indices])
    return video[keep_indices]
