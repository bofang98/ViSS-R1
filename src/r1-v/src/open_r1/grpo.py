...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        default_factory=lambda: ["accuracy", "transform", "format"],
        # default_factory=lambda: ["accuracy", "format"],
        metadata={"help": "List of reward functions. Possible values: 'accuracy', 'transform', 'format'"},
    )
...
def transform_reward(completions, trans_gt, **kwargs):
    # TODO: print("transform trans_gt here")
    ...
    # question_type = kwargs['problem_type'][0]     # this is question_type for real question
    contents = [completion[0]["content"] for completion in completions]

    rewards = []

    for content, sol in zip(contents, trans_gt):
    
        try:
            output_ans = extract_answer(content)        # "B" / ""
            gt_ans = extract_answer(sol)        # "A"
...
def format_reward(completions, **kwargs):
    ...
    pattern = r"<think>.*?</think>\s*<transform>.*?</transform>\s*<answer>.*?</answer>"     # with <transform> tag
