...
def prepare_SSL_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    """Prepare dataset example for training."""


    system_message = "You are an advanced multimedia analysis assistant."
    ...
    # transform question 
    trans_question = get_transform_question(example)

    # tranform solution
    trans_answer = "<transform>" + example['trans_ans'] + "</transform>"
    ...
    messages = [
        ...
        },
        {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": QUESTION_TEMPLATE.format(Question=question, Type_template=example['problem_type'], Trans_question=trans_question) 
                }
            ]
        },
        {
    ...
    return {"messages": messages, "label": example}
...
def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    for i, example in enumerate(examples):
        try:
            ...
            # apply tranformation according to dataset
            image_inputs, video_inputs = process_transformation(image_inputs, video_inputs, example["label"])
            ...
            texts.append(processor.apply_chat_template(example["messages"], tokenize=False))
            image_inputs, video_inputs, video_kwargs = process_vision_info(example["messages"], return_video_kwargs=True)
            print(f"Failed to process example {i}: {example}")
...
if __name__ == "__main__":
    ...
    prepared_dataset = [prepare_SSL_dataset(example) for example in dataset['train']]
