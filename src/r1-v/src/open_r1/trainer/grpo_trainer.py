...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def cosine_annealed_temp(self, initial_temp=1.0, final_temp=0.1, current_step=0, total_steps=5000):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if inputs[0]['data_type'] == 'image':
            input_copy[0]['content'][0]['image'] = "/gpubox03/ssd3/fangbo05/Video-R1-data" + inputs[0]['path'][1:] 
        elif inputs[0]['data_type'] == 'video':
            input_copy[0]['content'][0]['video'] = "/gpubox03/ssd3/fangbo05/Video-R1-data" + inputs[0]['path'][1:] 
        ...
        try:
            ...
            if inputs[0]['data_type'] == 'image':
                input_copy[0]['content'][0]['image'] = "/gpubox03/ssd3/fangbo05" + "/Video-R1-data" + '/Math/Multimath-300k/17ff4c7d14c388134de02381b1fc2824.png'
            elif inputs[0]['data_type'] == 'video':
                input_copy[0]['content'][0]['video'] = "/gpubox03/ssd3/fangbo05" + "/Video-R1-data" + '/LLaVA-Video-178K/liwei_youtube_videos/videos/youtube_video_2024/ytb_7nRmsEw7nsE.mp4'
                
            image_inputs, video_inputs, video_kwargs = process_vision_info(input_copy, return_video_kwargs=True)
        
        # prompt invlvoing pretext task, apply SSL transform to image and video
        trans_image, trans_video, trans_gt, trans_question = self.process_transformation(image_inputs, video_inputs)
        user_question = self.process_user_question(inputs)

        # pretext question, mainly for getting the pretext task reward
        QUESTION_TEMPLATE = (
        ...
        msg = {
            "prompt": 
               [{
                    ...
                    "content": [
                        {
                            "type": inputs[0]['data_type'],
                        },
                        {
                            "type": "text",
                            "text": QUESTION_TEMPLATE.format(
                                Type_template=TYPE_TEMPLATE[inputs[0]["problem_type"]], 
        ...
        prompt_trans = [maybe_apply_chat_template(msg, self.processing_class)["prompt"]]
        prompt_inputs = self.processing_class(
            text=copy.deepcopy(prompt_trans),
            images=trans_image,
            videos=trans_video,
            return_tensors="pt",
            padding=True,
        ...
        # Generate completions   o_1, o_2, ..., o_G?
        # self.generation_config.temperature = self.cosine_annealed_temp(current_step=self.iteration)
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            prompt_completion_ids = unwrapped_model.generate(**prompt_inputs, generation_config=self.generation_config)     # [8, 1000+]
        ...
        # if rank == 0: print(f"iteration: {self.iteration}\ntemperature: {self.generation_config.temperature:.3f}")
        ...
        per_token_kl = torch.exp(x_clamped) - x_clamped - 1     # calculate KL divergence
        
        if self.temporal and video_inputs:      # calculate rewards for shuffle generations
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            reward_kwargs['trans_gt'] = [trans_gt] * self.num_generations
            output_reward_func = reward_func(prompts=prompts, completions=completions, **reward_kwargs)     # accuracy reward ? 
            rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)


        # ablation on rewards_per_func [accuracy_reward, transform_reward, format_reward]
        # B.1 only transform reward + format reward, setting acc_reward as 0.0
        # rewards_per_func[:, 0] = 0.0
        # B.2 only acc reward + format reward, setting transform_reward as 0.0
        # rewards_per_func[:, 1] = 0.0
        ...
        if self.len_control:
            ...
            if len(selected_indices) > 1:     
                for idx in selected_indices:
                    if 320 <= lenth_list[idx] <= 768:
                        rewards[idx] += 0.2
        
        print(f"data type: {inputs[0]['data_type']}, rewards: {rewards}")
        ...
        per_token_loss = -per_token_loss        # without kl divergence
        ...
        self._metrics["temperature"].append(self.generation_config.temperature)
        
        return loss
        


    def process_user_question(self, inputs):
        example = inputs[0]
    ...
    def process_transformation(self, image_inputs, video_inputs):

        if image_inputs: 
            image = image_inputs[0]
            # trans_opt = random.choice(['rotate'])   # always rotate
            trans_opt = random.choice(['puzzle'])     # 0280
            if trans_opt == 'rotate':
                trans_image, trans_gt, trans_question = self.process_rotate_image(image)
            elif trans_opt == 'flip':
                trans_image, trans_gt, trans_question = self.process_flip_image(image)
            elif trans_opt == 'puzzle':
                trans_image, trans_gt, trans_question = self.process_puzzle_image(image)
            return [trans_image], None, trans_gt, trans_question

        elif video_inputs:
            video = video_inputs[0]     # [16, 3, H, W]
            trans_opt = random.choice(['arrow'])
            if trans_opt == 'rotate':
                trans_video, trans_gt, trans_question = self.process_rotate_video(video)
            elif trans_opt == 'shuffle':
                trans_video, trans_gt, trans_question = self.process_shuffle(video)
            elif trans_opt == 'arrow':
                trans_video, trans_gt, trans_question = self.process_arrow(video)
            return None, [trans_video], trans_gt, trans_question


    # rotate image
    def process_rotate_image(self, image):
        """
        """
        angles = [0, 90, 180, 270]
        ...
        return rotated_image, gt_answer, trans_question
    

    def process_flip_image(self, image):
        """
        """
        ...
        trans_question = (
            "Please review the input image and determine whether the image has been flipped. "
            "The possible options are:\n "
        ...
        return trans_image, gt_answer, trans_question


    def process_puzzle_image(self, image):
        """"""
        w, h = image.size   
        ...
        return new_img, gt_answer, trans_question


    # rotate transformation
    def process_rotate_video(self, video):
        """
        """
        angles = [0, 90, 180, 270]
        angle = random.choice(angles)
    ...
    def process_shuffle(self, video):
        """
    ...
    def manipulate_rewards(self, reward_func_cal):
        # reward_func_cal: [num_generations, 3].   [accuracy reward,  ssl reward, format reward]
        
        # exp style
        device = reward_func_cal.device
        sum_reward = torch.zeros(self.num_generations, 2).to(device)
        for i in range(reward_func_cal.shape[0]):   # num_generations
            q_acc = reward_func_cal[i, 0]
            tr_acc = reward_func_cal[i, 1]
            if tr_acc == 0 and q_acc < 0.5:      # all false
                sum_reward[i, 0] = q_acc
            elif tr_acc == 1 and q_acc < 0.5:    # tr correct but q false
                sum_reward[i, 0] = 0.2 + q_acc 
            elif tr_acc == 0 and q_acc >= 0.5:   # tr wrong but q true
                sum_reward[i, 0] = 0.7 if q_acc > 0.7 else q_acc
            elif tr_acc == 1 and q_acc >= 0.5:   # both true
                sum_reward[i, 0] = 1 + q_acc
        ...
        #     q_acc = reward_func_cal[i, 0]
        #     tr_acc = reward_func_cal[i, 1]
        #     if tr_acc == 0 and q_acc < 0.4:      # all false
        #         sum_reward[i, 0] = q_acc
        #     elif tr_acc == 1 and q_acc < 0.4:    # tr correct but q false
        #         sum_reward[i, 0] = 1.4 + q_acc 
        #     elif tr_acc == 0 and q_acc >= 0.4:   # tr wrong but q true
        #         sum_reward[i, 0] = 1.8 + 0.5*(q_acc - 1.0)
        #     elif tr_acc == 1 and q_acc >= 0.4:   # both true
        #         sum_reward[i, 0] = 2.0 + 0.5*(q_acc - 1.0)

        # TODO: uniform style

        # TODO: equal style
        
        sum_reward[:, 1] = reward_func_cal[:, 2]        # format value
        return sum_reward
