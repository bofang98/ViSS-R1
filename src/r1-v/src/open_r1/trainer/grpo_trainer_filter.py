...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with torch.inference_mode():
            with unwrap_model_for_generation(self.ref_model, self.accelerator) as unwrapped_ref_model:
                fix_completion_ids = unwrapped_ref_model.generate(**prompt_inputs, generation_config=self.generation_config)
                prompt_length = prompt_ids.size(1)
                fake_prompt_ids = fix_completion_ids[:, :prompt_length]
                fake_completion_ids = fix_completion_ids[:, prompt_length:]

        # Decode the generated completions
        fake_completions = self.processing_class.batch_decode(fake_completion_ids, skip_special_tokens=True)
        if is_conversational(inputs[0]):
            completions_ = [[{"role": "assistant", "content": completion}] for completion in fake_completions]

        device = self.accelerator.device
        # Compute the rewards
        prompts_copy = [prompt for prompt in prompts for _ in range(self.num_generations)]
        rewards_per_func_fake = torch.zeros(len(prompts_copy), len(self.reward_funcs), device=device)
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            output_reward_func = reward_func(prompts=prompts_copy, completions=completions_, **reward_kwargs)
            rewards_per_func_fake[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)

        # judge & save
        acc_reward = rewards_per_func_fake[:, 0]
        
        num_zeros = (acc_reward == 0).sum().item()
        num_ones = (acc_reward == 1).sum().item()

        if num_zeros != len(acc_reward) and num_ones != len(acc_reward):
            with open('/root/paddlejob/workspace/env_run/log/Qwen2.5-VL-7B-debug/value_data.txt', 'a') as f:
                f.write(f"{{'problem_id': {inputs[0]['problem_id']}, 'num_0': {num_zeros}, 'num_1': {num_ones}}}\n")
        
        
        del completions_, fix_completion_ids, fake_completion_ids, fake_prompt_ids, rewards_per_func_fake, fake_completions, prompts_copy
