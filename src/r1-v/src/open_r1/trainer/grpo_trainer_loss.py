...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if inputs[0]['data_type'] == 'image':
            ...
            input_copy[0]['content'][0]['fps'] = 1.0
            input_copy[0]['content'][0]['min_pixels'] = 128 * 28 * 28
        ...
        try:
            ...
            image_inputs, video_inputs = process_vision_info(input_copy)
        ...
        # probabilistic sampling with different rewards
        sample_video, sample_rewards = self.video_mask_loss_dist(rank, inputs, video_inputs)    # [video_pos, video_neg], [reward_pos, reward_neg]
        

        prompt_inputs = self.processing_class(
            text=copy.deepcopy(prompts_text),
            images=image_inputs,
            videos=[sample_video[0]],       # positive video clip w larger reward (low loss)
            return_tensors="pt",
            padding=True,
            padding_side="left",
        ...
        if self.temporal and video_inputs:
            ...
            # mimic shuffled video inputs
            shuffled_prompt_inputs = self.processing_class(
                text=copy.deepcopy(prompts_text),
                images=image_inputs,
                videos=[sample_video[1]],      # video clip w/ smaller reward (high loss)
                return_tensors="pt",
                padding=True,
                padding_side="left",
                add_special_tokens=False,
        ...
        if self.temporal and video_inputs:
            ...
            if acc_mean >= shuffled_acc_mean:
                ...
                temporal_rewards_per_func[mask, 0] = temporal_rewards_per_func[mask, 0] + 0.3       # Fix to 0.3
                # temporal_rewards_per_func[mask, 0] = temporal_rewards_per_func[mask, 0] + sample_rewards[0]     # Adaptively reward
    ...
    def video_mask_loss_dist(self, rank, info, video_inputs):
        problem_id = info[0]['problem_id']
        loss_path = os.path.join("/ssd2/vis/wuwenhao/fangbo/Video-R1-main/preprocess/output_mask_loss", f"{problem_id}.json")
        with open(loss_path, 'r') as f:
            data = json.load(f)

        num_frames = len(json.loads(data[0]['mask']))  
        assert num_frames == video_inputs[0].shape[0], f"{problem_id} should have same frame numbers {num_frames} vs. {video_inputs[0].shape[0]} with fps 1"

        loss_sum = torch.zeros(num_frames)
        count = torch.zeros(num_frames)

        for item in data:
            mask = torch.tensor(json.loads(item['mask']), dtype=torch.float32) 
            loss = float(item['loss'])
            
            loss_sum += mask * loss
            count += mask

        # distribution loss of frames against specific query
        loss_distribution = loss_sum / count

        # normalization and inverse to sampling probability (also rewards)
        min_val = loss_distribution.min()
        max_val = loss_distribution.max()
        normalized_reward = 0.5 - (loss_distribution - min_val) / (max_val - min_val) * 0.5     # scale to [0,0.5] and reverse
        # prob_reward = torch.exp(-loss_distribution)
        
        # expand
        reward, video_inputs = self.expand_to_32(normalized_reward, video_inputs)
        masks = self.generate_masks(len(reward), num_masks=16, ones_per_mask=16, mode='stride')

        global_mask = torch.zeros(masks.shape[1], dtype=torch.int32)
        for i in range(1, 16+1):
            pos = int(i * (masks.shape[1] / 17))
            global_mask[pos] = 1

        # calculate the reward
        mean_reward = [torch.mean(reward[mask==1]) for mask in masks]
        max_avg_reward = max(mean_reward)
        max_mask = masks[mean_reward.index(max_avg_reward)]
        min_avg_reward = min(mean_reward)
        min_mask = masks[mean_reward.index(min_avg_reward)]
        
        # video_pos = video_inputs[0][max_mask==1]
        # video_neg = video_inputs[0][min_mask==1]

        # global uniform mask
        global_mask_reward = torch.mean(reward[global_mask==1])
        if torch.abs(max_avg_reward - global_mask_reward) > torch.abs(min_avg_reward - global_mask_reward):
            video_pos = video_inputs[0][max_mask==1]
            video_neg = video_inputs[0][global_mask==1]
            diff_reward = torch.abs(max_avg_reward - global_mask_reward)
        else:
            video_pos = video_inputs[0][global_mask==1]
            video_neg = video_inputs[0][min_mask==1]
            diff_reward = torch.abs(min_avg_reward - global_mask_reward)

        return [video_pos, video_neg], [diff_reward, torch.tensor(0.0)]
    

    def expand_to_32(self, reward, video_inputs):
        orig_len = reward.shape[0]
        if orig_len >= 32: return reward, video_inputs
        idxs = torch.linspace(0, orig_len-1, 32).round().long()
        reward_expanded = reward[idxs]
        video_inputs = [video_inputs[0][idxs]]
        return reward_expanded, video_inputs
    

    def generate_masks(self, n, num_masks=4, ones_per_mask=16, mode='stride'):
        masks = []
    ...
    def pad_completion_ids(self, batch_prompt_completion_ids, pad_token_id=151643):
        device = batch_prompt_completion_ids[0].device

        max_len = max(t.shape[1] for t in batch_prompt_completion_ids)

        padded_tensors = []
        for t in batch_prompt_completion_ids:
            pad_len = max_len - t.shape[1]
            if pad_len > 0:
                pad_tensor = torch.full((t.shape[0], pad_len), pad_token_id, dtype=t.dtype, device=device)
                t_padded = torch.cat([t, pad_tensor], dim=1)
            else:
                t_padded = t
