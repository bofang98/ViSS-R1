...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def cosine_annealed_temp(self, initial_temp=1.0, final_temp=0.1, current_step=0, total_steps=1000):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        trans_image, trans_video, trans_gt, trans_prompt = self.process_transformation(image_inputs, video_inputs)
        ...
        prompts = [msg['prompt'][0] for _ in range(self.num_generations)]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            # replace solution with trans_gt
            reward_kwargs['problem_type'] = []
            reward_kwargs['problem_type'].extend(['multiple choice'] * self.num_generations)
            reward_kwargs['solution'] = []
            reward_kwargs['solution'].extend([trans_gt] * self.num_generations)
    ...
    def process_transformation(self, image_inputs, video_inputs):
        if image_inputs: 
            ...
            trans_opt = random.choice(['flip'])
            if trans_opt == 'rotate':
                trans_image, trans_gt, trans_prompt = self.process_rotate_image(image)
            elif trans_opt == 'flip':
                trans_image, trans_gt, trans_prompt = self.process_flip_image(image)
            elif trans_opt == 'puzzle':
                trans_image, trans_gt, trans_prompt = self.process_puzzle_image(image)
            return [trans_image], None, trans_gt, trans_prompt
        ...
        if video_inputs:
            ...
            if trans_opt == 'rotate':
                trans_video, trans_gt, trans_prompt = self.process_rotate_video(video)
            elif trans_opt == 'shuffle':
                trans_video, trans_gt, trans_prompt = self.process_shuffle(video)
            elif trans_opt == 'arrow':
                trans_video, trans_gt, trans_prompt = self.process_arrow(video)
            return None, [trans_video], trans_gt, trans_prompt
    ...
    def process_flip_image(self, image):
        ...
        if angle == 'horizontal':
            ...
            gt_answer = "<answer>B</answer>"
        elif angle == 'vertical':
            trans_image = image.transpose(Image.FLIP_TOP_BOTTOM)
            gt_answer = "<answer>C</answer>"
        else:
            trans_image = image
            gt_answer = "<answer>A</answer>"
        trans_prompt = (
            ...
            # "Please provide only the single option letter (e.g., A, B, etc.) within your answers."
            # "Afterward, restore the video to its original orientation and proceed to answer the following question.\n\n"
    ...
    def process_puzzle_image(self, image):
        ...
        trans_prompt = (
            "Please carefully review the transformed image. The image has been evenly divided into four equal-sized patches, labeled as follows:\n "
    ...
    def process_rotate_video(self, video):
        ...
        trans_prompt = (
            ...
            "A. 0°\n B. 90°\n C. 180°\n D. 270°\n"
            "Note: The rotation is applied in the counterclockwise (anticlockwise) direction and 0° corresponds to no rotation.\n"
            "Please focus on the spatial cues of video contents and think about this question as if you were a human pondering deeply. "
            "Engage in an internal dialogue using expressions such as 'let me think', 'wait', 'Hmm', 'oh, I see', 'let's break it down', etc, or other natural language thought expressions. "
            "It's encouraged to include self-reflection or verification in the reasoning process. "
            "Provide your detailed reasoning between the <think> </think> tags, and then give your final answer between the <answer> </answer> tags. "
            "Please provide only the single option letter (e.g., A, B, C, D, etc.) within the <answer> </answer> tags."
    ...
    def process_shuffle(self, video):
        ...
        groups = [
            ...
            list(range(8, 12)),  # [8,9,10,11]
            list(range(12, 16))  # [12,13,14,15]
        ]
        answer_map = {
            (0, 1): '<answer>A</answer>',
            (0, 2): '<answer>B</answer>',
        ...
        gt_answer = answer_map[(idx1, idx2)]
        trans_prompt = (
            "Please review the input video clip. The video originally consists of 16 frames, divided into 4 consecutive groups as follows:\n "
            "Group 1: Frames [1, 2, 3, 4]\n Group 2: Frames [5, 6, 7, 8]\n Group 3: Frames [9, 10, 11, 12]\n Group 4: Frames [13, 14, 15, 16]\n"
            ...
            # "Provide your answer within the <answer> </answer> tags. "
            # "Please provide only the single option letter (e.g., A, B, C, D, E, F, etc.) within your answers."
            # "Afterward, restore the video to its original order and proceed to answer the following question.\n\n"
